# CLAUDE.md — Cérebro Pessoal de Adriana Bessa

> Instruções pro Claude Code / Cowork / OpenClaw operar neste cérebro pessoal.

## Contexto

Este é o **cérebro pessoal individual** de **Adriana Bessa**, sócio-diretor da Grupo VAB. Agente default: **Ada**.
...
O Claude atua como **estação de trabalho individual** de Adriana Bessa:
...
Se Adriana Bessa é diretor da Grupo VAB, recomenda-se clonar os 3 cérebros lado a lado:

```
~/brains/
├── grupo-vab-second-brain/        (cérebro do time)
├── grupo-vab-diretoria/           (cérebro da diretoria)
└── grupo-vab-ada/     (este — pessoal)
```

Ao abrir **qualquer uma** dessas 3 pastas no Claude Code/Cowork, a ferramenta deve:

1. Ler este `CLAUDE.md` (deste cérebro pessoal)
2. Se existirem `../grupo-vab-second-brain/CLAUDE.md` e `../grupo-vab-diretoria/CLAUDE.md`, também carregar esses
...
- `agentes/ada/` — config + memory local (daily notes)
...
3. **Daily notes automáticas em `agentes/ada/memory/YYYY-MM-DD.md`** — criadas pelo agente durante uso.
